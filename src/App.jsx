...
function SidebarFeeControl({ 
  ...
  const secondaryText = mode === 'pct' 
    ? (isGuarantee ? "Sur capital emprunté" : `≈ ${Math.round(amount).toLocaleString()} €`) 
    : (mode === 'eur' && type === 'percent' ? `${rate.toFixed(precision)} %` : "");

  return (
    <div className="group py-0.5">
      <div className="flex justify-between items-center min-h-[28px] gap-2">
        {/* Partie gauche : Label + Bouton Reset repositionné */}
        <div className="flex items-center gap-1.5 min-w-0">
          <span className="text-[10px] font-black text-slate-500 uppercase tracking-tight group-hover:text-slate-400 transition-colors truncate">
            {label}
          </span>
          {resetValue !== undefined && (
            <button 
              ...
              className="w-5 h-5 shrink-0 flex items-center justify-center text-slate-600 hover:text-white hover:bg-slate-800 rounded-md transition-all opacity-40 group-hover:opacity-100"
              title="Réinitialiser"
            >
              <RotateCcw size={10} />
            </button>
        ...
        {/* Partie droite : Toggle (si pct) + Input élargi et parfaitement aligné */}
        <div className="flex items-center gap-1.5 shrink-0 justify-end">
          {type === 'percent' && (
            <button
              onClick={handleModeToggle}
              className={cn(
                "w-6 h-6 flex items-center justify-center text-[9px] font-black rounded-lg transition-all border shadow-sm shrink-0",
                mode === 'pct'
          ...
          {/* Input block - largeur fixe généreuse de w-32 pour un alignement parfait */}
          <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
            <button
              ...
              className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all"
            >
              <Minus size={10} />
            </button>
            ...
            <input
              ...
              className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5"
            />
      ...
      {/* Texte secondaire sous l'input, réduit l'interligne en disparaissant si vide */}
      {secondaryText && (
        <div className="flex justify-end pr-1 mt-0.5 leading-none">
          <span className="text-[8px] text-slate-600 font-bold uppercase tracking-tight tabular-nums">
            {secondaryText}
          </span>
        </div>
      )}
...
function App() {
  ...
  const [globalSettings, setGlobalSettings] = useState(() => {
    const loaded = initialState?.globalSettings || FACTORY_SETTINGS;
    return {
      ...loaded,
      rendementDefaults: loaded.rendementDefaults || FACTORY_SETTINGS.rendementDefaults
    };
  });
  ...
  return (
    <div className="h-screen bg-[#f8fafc] flex overflow-hidden flex-col md:flex-row">
              ...
              <div className="bg-slate-800/40 p-6 rounded-[2rem] space-y-6 border border-slate-700/50">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center gap-2">
                    <div className="flex items-center gap-1.5 min-w-0">
                      <button onClick={() => updateCurrentScenario('apport', Math.round(currentResults.minApportAt35))} className="bg-brand-secondary/20 text-brand-secondary px-1.5 py-0.5 rounded text-[8px] font-black uppercase hover:bg-brand-secondary hover:text-white transition-all shrink-0">Opti</button>
                      <button onClick={() => updateCurrentScenario('apport', Math.round(currentResults.notaryFees))} className="bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded text-[8px] font-black uppercase hover:bg-slate-600 transition-all shrink-0">Notaire</button>
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate ml-0.5">Apport</label>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0 justify-end">
                      <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
                        <button onClick={() => updateCurrentScenario('apport', Math.max(0, currentScenario.apport - 1000))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                          <Minus size={10} />
                        </button>
                        <input type="number" step="1000" value={currentScenario.apport} onChange={(e) => updateCurrentScenario('apport', Number(e.target.value))} className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5" />
                        <button onClick={() => updateCurrentScenario('apport', Math.min(1000000, currentScenario.apport + 1000))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                          <Plus size={10} />
                        </button>
                      </div>
                      <span className="text-[10px] font-black text-slate-500 w-5 shrink-0 text-left">€</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center gap-2 group/row">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate">Taux</label>
                        <button onClick={() => updateCurrentScenario('rate', 3.8)} className="w-5 h-5 shrink-0 flex items-center justify-center text-slate-600 hover:text-white hover:bg-slate-800 rounded-md transition-all opacity-40 group-hover/row:opacity-100" title="Réinitialiser">
                          <RotateCcw size={10} />
                        </button>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0 justify-end">
                        <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
                          <button onClick={() => updateCurrentScenario('rate', Number(Math.max(1, currentScenario.rate - 0.05).toFixed(2)))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                            <Minus size={10} />
                          </button>
                          <input type="number" step="0.01" value={currentScenario.rate} onChange={(e) => updateCurrentScenario('rate', Number(e.target.value))} className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5" />
                          <button onClick={() => updateCurrentScenario('rate', Number(Math.min(6, currentScenario.rate + 0.05).toFixed(2)))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                            <Plus size={10} />
                          </button>
                        </div>
                        <span className="text-[10px] font-black text-slate-500 w-5 shrink-0 text-left">%</span>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center gap-2 group/row">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate">Ass.</label>
                        <button onClick={() => updateCurrentScenario('insurance', 0.5)} className="w-5 h-5 shrink-0 flex items-center justify-center text-slate-600 hover:text-white hover:bg-slate-800 rounded-md transition-all opacity-40 group-hover/row:opacity-100" title="Réinitialiser">
                          <RotateCcw size={10} />
                        </button>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0 justify-end">
                        <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
                          <button onClick={() => updateCurrentScenario('insurance', Number(Math.max(0, currentScenario.insurance - 0.01).toFixed(2)))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                            <Minus size={10} />
                          </button>
                          <input type="number" step="0.01" value={currentScenario.insurance} onChange={(e) => updateCurrentScenario('insurance', Number(e.target.value))} className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5" />
                          <button onClick={() => updateCurrentScenario('insurance', Number(Math.min(2, currentScenario.insurance + 0.01).toFixed(2)))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                            <Plus size={10} />
                          </button>
                        </div>
                        <span className="text-[10px] font-black text-slate-500 w-5 shrink-0 text-left">%</span>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center gap-2">
                      <label className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate">Durée</label>
                      <div className="flex items-center gap-1.5 shrink-0 justify-end">
                        <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
                          <button onClick={() => updateCurrentScenario('duration', Math.max(10, currentScenario.duration - 1))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                            <Minus size={10} />
                          </button>
                          <input type="number" value={currentScenario.duration} onChange={(e) => updateCurrentScenario('duration', Number(e.target.value))} className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5" />
                          <button onClick={() => updateCurrentScenario('duration', Math.min(25, currentScenario.duration + 1))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                            <Plus size={10} />
                          </button>
                        </div>
                        <span className="text-[10px] font-black text-slate-500 w-5 shrink-0 text-left">ans</span>
                      </div>
                    </div>
                  ...
                  <AnimatePresence>
                    {isFraisAnnexesOpen && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden space-y-4 pt-2 pb-1">
                        <div>
                          <div className="flex justify-between items-center mb-1 gap-2 group/row">
                            <div className="flex items-center gap-1.5 min-w-0">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate">Frais Banque</label>
                              <button onClick={() => updateCurrentScenario('bankFee', undefined)} className="w-5 h-5 shrink-0 flex items-center justify-center text-slate-600 hover:text-white hover:bg-slate-800 rounded-md transition-all opacity-40 group-hover/row:opacity-100" title="Réinitialiser">
                                <RotateCcw size={10} />
                              </button>
                            </div>
                            <div className="flex items-center gap-1.5 shrink-0 justify-end">
                              <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
                                <button onClick={() => updateCurrentScenario('bankFee', Math.max(0, (currentScenario.bankFee ?? 1500) - 100))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                                  <Minus size={10} />
                                </button>
                                <input type="number" step="100" value={currentScenario.bankFee ?? 1500} onChange={(e) => updateCurrentScenario('bankFee', Number(e.target.value))} className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5" />
                                <button onClick={() => updateCurrentScenario('bankFee', Math.min(10000, (currentScenario.bankFee ?? 1500) + 100))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                                  <Plus size={10} />
                                </button>
                              </div>
                              <span className="text-[10px] font-black text-slate-500 w-5 shrink-0 text-left">€</span>
                        ...
                        <div className="space-y-4">
                          <div>
                            <div className="flex justify-between items-center mb-1 gap-2 group/row">
                              <div className="flex items-center gap-1.5 min-w-0">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate">Garantie</label>
                                <button onClick={() => updateCurrentScenario('guaranteeFee', undefined)} className="w-5 h-5 shrink-0 flex items-center justify-center text-slate-600 hover:text-white hover:bg-slate-800 rounded-md transition-all opacity-40 group-hover/row:opacity-100" title="Réinitialiser">
                                  <RotateCcw size={10} />
                                </button>
                              </div>
                              <div className="flex items-center gap-1.5 shrink-0 justify-end">
                                <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
                                  <button onClick={() => updateCurrentScenario('guaranteeFee', Math.max(0, Math.round(currentScenario.guaranteeFee ?? currentResults.guaranteeFee) - 100))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                                    <Minus size={10} />
                                  </button>
                                  <input type="number" step="100" value={Math.round(currentScenario.guaranteeFee ?? currentResults.guaranteeFee)} onChange={(e) => updateCurrentScenario('guaranteeFee', Number(e.target.value))} className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5" />
                                  <button onClick={() => updateCurrentScenario('guaranteeFee', Math.min(20000, Math.round(currentScenario.guaranteeFee ?? currentResults.guaranteeFee) + 100))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                                    <Plus size={10} />
                                  </button>
                                </div>
                                <span className="text-[10px] font-black text-slate-500 w-5 shrink-0 text-left">€</span>
                          ...
                          <div>
                            <div className="flex justify-between items-center mb-1 gap-2 group/row">
                              <div className="flex items-center gap-1.5 min-w-0">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-tight truncate">Courtage</label>
                                <button onClick={() => updateCurrentScenario('brokerFee', undefined)} className="w-5 h-5 shrink-0 flex items-center justify-center text-slate-600 hover:text-white hover:bg-slate-800 rounded-md transition-all opacity-40 group-hover/row:opacity-100" title="Réinitialiser">
                                  <RotateCcw size={10} />
                                </button>
                              </div>
                              <div className="flex items-center gap-1.5 shrink-0 justify-end">
                                <div className="flex items-center justify-between bg-slate-900/50 rounded-lg border border-slate-700/50 p-0.5 shadow-inner w-32 shrink-0">
                                  <button onClick={() => updateCurrentScenario('brokerFee', Math.max(0, (currentScenario.brokerFee ?? 4100) - 100))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                                    <Minus size={10} />
                                  </button>
                                  <input type="number" step="100" value={currentScenario.brokerFee ?? 4100} onChange={(e) => updateCurrentScenario('brokerFee', Number(e.target.value))} className="bg-transparent text-white text-[11px] font-black text-center outline-none tabular-nums w-full px-0.5" />
                                  <button onClick={() => updateCurrentScenario('brokerFee', Math.min(10000, (currentScenario.brokerFee ?? 4100) + 100))} className="w-6 h-6 shrink-0 flex items-center justify-center text-slate-500 hover:text-white hover:bg-slate-800 rounded-md transition-all">
                                    <Plus size={10} />
                                  </button>
                                </div>
                                <span className="text-[10px] font-black text-slate-500 w-5 shrink-0 text-left">€</span>
      ...
      <AnimatePresence>
        {isSettingsOpen && (
          <div className="fixed inset-0 z-[110] bg-slate-950 flex flex-col overflow-hidden">
            <motion.div 
              initial={{ opacity: 0, y: 30 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 30 }}
              className="flex-1 flex flex-col overflow-hidden max-w-[1600px] w-full mx-auto"
            >
              {/* Header Plein Écran */}
              <header className="shrink-0 border-b border-slate-800/80 bg-slate-900/50 px-6 py-5 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-brand-primary/20 flex items-center justify-center text-brand-primary">
                    <Settings size={24} />
                  </div>
                  <div>
                    <h2 className="text-xl md:text-2xl font-black text-white">Configuration Master</h2>
                    <p className="text-[10px] md:text-xs text-slate-400 font-bold uppercase tracking-widest mt-0.5">Définition des variables par défaut de l'application</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <button 
                    ...
                    className="px-6 py-3 bg-brand-primary text-white rounded-xl font-black text-xs uppercase tracking-wider hover:bg-brand-primary/90 transition-all shadow-lg shadow-brand-primary/20 flex items-center gap-2"
                  >
                    <span>Sauvegarder & Appliquer</span>
                  </button>
                  <button 
                    onClick={() => setIsSettingsOpen(false)} 
                    className="w-10 h-10 rounded-xl bg-slate-800/80 border border-slate-700/50 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-800 transition-colors"
                    title="Fermer sans appliquer"
                  >
              ...
              {/* Corps Défilant Structuré */}
              <div className="flex-1 overflow-y-auto px-6 py-8 custom-scrollbar">
                <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8">
                  
                  {/* COLONNE GAUCHE : REVENUS & LOGEMENT ACTUEL */}
                  <div className="space-y-8">
                    
                    {/* Jess */}
                    <section className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 space-y-6 backdrop-blur-sm">
                      <h3 className="text-[10px] font-black text-rose-400 uppercase tracking-widest flex items-center gap-2">
                        <span className="w-6 h-px bg-rose-500/30" /> Rémunération Jess (Salariat)
                      </h3>
                      <div className="grid grid-cols-2 gap-3">
                        {[
                          ...
                          <div key={f.key} className="space-y-1 bg-slate-950/40 p-3 rounded-xl border border-slate-800/50">
                            ...
                            <input
                              ...
                              className="w-full bg-transparent text-white font-black text-sm focus:text-rose-400 outline-none transition-colors"
                            />
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="pt-2 border-t border-slate-800/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                        <div className="space-y-0.5">
                          <span className="text-[9px] font-bold text-rose-300 uppercase block">Revenu calculé de référence</span>
                          <span className="text-[10px] text-slate-500 block">Prudent : <strong className="text-rose-400">{Math.round(jessCalculated.netSansBonus)} €</strong> | Opti : <strong className="text-emerald-400">{Math.round(jessCalculated.netAvecBonus)} €</strong></span>
                        </div>
                        
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                          <div className="space-y-0.5 grow sm:grow-0">
                            <span className="text-[8px] font-bold text-slate-500 uppercase block">Retenu Banque</span>
                            <input
                              type="number"
                              value={globalSettings.incomeJess}
                              onChange={(e) => setGlobalSettings({...globalSettings, incomeJess: Number(e.target.value)})}
                              className="w-full sm:w-24 bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-white font-black text-xs focus:border-rose-400 outline-none text-right"
                            />
                          </div>
                          <div className="flex flex-col gap-1 self-end">
                            <button onClick={() => setGlobalSettings({...globalSettings, incomeJess: Math.round(jessCalculated.netSansBonus)})} className="text-[8px] font-black bg-rose-500/10 text-rose-400 px-2 py-0.5 rounded border border-rose-500/20 hover:bg-rose-500 hover:text-white transition-all" title="Appliquer la vue prudente">Prudent</button>
                            <button onClick={() => setGlobalSettings({...globalSettings, incomeJess: Math.round(jessCalculated.netAvecBonus)})} className="text-[8px] font-black bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/20 hover:bg-emerald-500 hover:text-white transition-all" title="Appliquer la vue optimiste">Optimiste</button>
                          </div>
                        </div>
                      </div>
                    </section>
                    ...
                    <section className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 space-y-6 backdrop-blur-sm">
                      <h3 className="text-[10px] font-black text-sky-400 uppercase tracking-widest flex items-center gap-2">
                        <span className="w-6 h-px bg-sky-500/30" /> Rémunération Renaud (BNC Indépendant)
                      </h3>
                      <div className="grid grid-cols-3 gap-3">
                        <div className="space-y-1 bg-slate-950/40 p-3 rounded-xl border border-slate-800/50">
                          <label className="text-[9px] font-bold text-slate-400 uppercase">Encaissé HT /an</label>
                          <input type="number" value={globalSettings.renaudBnc?.collectedHT ?? 0} onChange={(e) => setGlobalSettings({...globalSettings, renaudBnc: { ...globalSettings.renaudBnc, collectedHT: Number(e.target.value) }})} className="w-full bg-transparent text-white font-black text-xs focus:text-sky-400 outline-none transition-colors" />
                        </div>
                        <div className="space-y-1 bg-slate-950/40 p-3 rounded-xl border border-slate-800/50">
                          <label className="text-[9px] font-bold text-slate-400 uppercase">Facturé HT /an</label>
                          <input type="number" value={globalSettings.renaudBnc?.billedHT ?? 0} onChange={(e) => setGlobalSettings({...globalSettings, renaudBnc: { ...globalSettings.renaudBnc, billedHT: Number(e.target.value) }})} className="w-full bg-transparent text-white font-black text-xs focus:text-sky-400 outline-none transition-colors" />
                        </div>
                        <div className="space-y-1 bg-slate-950/40 p-3 rounded-xl border border-slate-800/50">
                          <label className="text-[9px] font-bold text-slate-400 uppercase">Prévisionnel HT</label>
                          <input type="number" value={globalSettings.renaudBnc?.forecastHT ?? 0} onChange={(e) => setGlobalSettings({...globalSettings, renaudBnc: { ...globalSettings.renaudBnc, forecastHT: Number(e.target.value) }})} className="w-full bg-transparent text-white font-black text-xs focus:text-emerald-400 outline-none transition-colors" />
                        </div>
                      </div>

                      <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between gap-3">
                        <span className="text-[9px] font-bold text-slate-400 uppercase">Retenu Banque mensuel</span>
                        <div className="flex items-center gap-1.5">
                          <input type="number" value={globalSettings.incomeRenaud} onChange={(e) => setGlobalSettings({...globalSettings, incomeRenaud: Number(e.target.value)})} className="w-24 bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1.5 text-white font-black text-xs focus:border-sky-400 outline-none text-right" />
                    ...
                    {/* Logement Actuel */}
                    <section className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 flex items-center justify-between gap-4 backdrop-blur-sm">
                      <div className="space-y-0.5">
                        <h3 className="text-[10px] font-black text-amber-400 uppercase tracking-widest">Logement Actuel (Locataire)</h3>
                        <p className="text-[9px] text-slate-500">Loyer de référence pour les comparaisons d'enrichissement</p>
                      </div>
                      <div className="flex items-center gap-1.5 shrink-0 bg-slate-950/40 p-2.5 rounded-xl border border-slate-800/50">
                        <span className="text-[9px] font-bold text-slate-500 uppercase">Loyer</span>
                        <input type="number" value={globalSettings.rent} onChange={(e) => setGlobalSettings({...globalSettings, rent: Number(e.target.value)})} className="w-20 bg-transparent text-white font-black text-sm outline-none text-right focus:text-amber-400" />
                  ...
                  {/* COLONNE DROITE : PATRIMOINE & HYPOTHÈSES RENDEMENT */}
                  <div className="space-y-6">
                    
                    <div className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 space-y-6 backdrop-blur-sm">
                      <div className="pb-2 border-b border-slate-800/80">
                        <h3 className="text-[10px] font-black text-emerald-400 uppercase tracking-widest flex items-center gap-2">
                          <span className="w-6 h-px bg-emerald-500/30" /> Hypothèses Rendement & Épargne
                        </h3>
                      </div>

                      {/* 1. Patrimoine Initial */}
                      <div className="bg-slate-950/40 border border-slate-800/50 rounded-2xl p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider">1 · Patrimoine Initial</span>
                          <button 
                            onClick={() => {
                              setGlobalSettings(prev => ({
                                ...prev,
                                epargneTotale: 200000,
                                rendementDefaults: { ...(prev.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), epargneTotale: 200000, matelas: 20000, travaux: 30000 }
                              }));
                            }}
                            className="text-slate-600 hover:text-emerald-400 text-[10px] px-1 transition-colors font-bold"
                            title="Réinitialiser les montants initiaux"
                          >↺</button>
                        </div>

                        <div className="grid grid-cols-3 gap-2.5">
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Épargne totale</label>
                            <input
                              type="number" step="5000"
                              value={globalSettings.epargneTotale ?? FACTORY_SETTINGS.epargneTotale}
                              onChange={(e) => {
                                const val = Number(e.target.value);
                                setGlobalSettings(prev => ({ ...prev, epargneTotale: val, rendementDefaults: { ...(prev.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), epargneTotale: val } }));
                              }}
                              className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-emerald-400"
                            />
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Matelas (Court)</label>
                            <input
                              type="number" step="1000"
                              value={globalSettings.rendementDefaults?.matelas ?? 20000}
                              onChange={(e) => {
                                const val = Number(e.target.value);
                                setGlobalSettings(prev => ({ ...prev, rendementDefaults: { ...(prev.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), matelas: val } }));
                              }}
                              className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-emerald-400"
                            />
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Provision travaux</label>
                            <input
                              type="number" step="1000"
                              value={globalSettings.rendementDefaults?.travaux ?? 30000}
                              onChange={(e) => {
                                const val = Number(e.target.value);
                                setGlobalSettings(prev => ({ ...prev, rendementDefaults: { ...(prev.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), travaux: val } }));
                              }}
                              className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-emerald-400"
                            />
                          </div>
                      ...
                      <div className="bg-slate-950/40 border border-slate-800/50 rounded-2xl p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[9px] font-black text-sky-400 uppercase tracking-wider">2 · Court Terme</span>
                          <button 
                            onClick={() => {
                              setGlobalSettings(prev => ({ ...prev, rendementDefaults: { ...(prev.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), rendementCourt: 1.5, bonusCourt: 0, partCourt: 20 } }));
                            }}
                            className="text-slate-600 hover:text-sky-400 text-[10px] px-1 transition-colors font-bold"
                            title="Réinitialiser hypothèses Court"
                          >↺</button>
                        </div>

                        <div className="grid grid-cols-3 gap-2.5">
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Rendement net</label>
                            <div className="flex items-center">
                              <input type="number" step="0.1" value={globalSettings.rendementDefaults?.rendementCourt ?? 1.5} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), rendementCourt: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-sky-400" />
                              <span className="text-[9px] text-slate-500 ml-0.5">%</span>
                            </div>
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Bonus Rendement</label>
                            <div className="flex items-center">
                              <span className="text-[9px] text-slate-500 mr-0.5">+</span>
                              <input type="number" step="0.1" value={globalSettings.rendementDefaults?.bonusCourt ?? 0} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), bonusCourt: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-sky-400" />
                              <span className="text-[9px] text-slate-500 ml-0.5">%</span>
                            </div>
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">% Épargne Mens.</label>
                            <div className="flex items-center">
                              <input type="number" step="5" min="0" max="100" value={globalSettings.rendementDefaults?.partCourt ?? 20} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), partCourt: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-sky-400" />
                              <span className="text-[9px] text-slate-500 ml-0.5">%</span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="bg-slate-950/40 border border-slate-800/50 rounded-2xl p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[9px] font-black text-emerald-400 uppercase tracking-wider">3 · Moyen Terme</span>
                          <button 
                            onClick={() => {
                              setGlobalSettings(prev => ({ ...prev, rendementDefaults: { ...(prev.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), montantMoyen: 30000, rendementMoyen: 2.5, bonusMoyen: 0, partMoyen: 20 } }));
                            }}
                            className="text-slate-600 hover:text-emerald-400 text-[10px] px-1 transition-colors font-bold"
                            title="Réinitialiser hypothèses Moyen"
                          >↺</button>
                        </div>

                        <div className="grid grid-cols-4 gap-2">
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Cap. Alloué</label>
                            <input type="number" step="1000" value={globalSettings.rendementDefaults?.montantMoyen ?? 30000} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), montantMoyen: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-emerald-400" />
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Rendement</label>
                            <div className="flex items-center">
                              <input type="number" step="0.1" value={globalSettings.rendementDefaults?.rendementMoyen ?? 2.5} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), rendementMoyen: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-emerald-400" />
                              <span className="text-[8px] text-slate-500">%</span>
                            </div>
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Bonus</label>
                            <div className="flex items-center">
                              <span className="text-[8px] text-slate-500">+</span>
                              <input type="number" step="0.1" value={globalSettings.rendementDefaults?.bonusMoyen ?? 0} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), bonusMoyen: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-emerald-400" />
                              <span className="text-[8px] text-slate-500">%</span>
                            </div>
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">% Épargne</label>
                            <div className="flex items-center">
                              <input type="number" step="5" min="0" max="100" value={globalSettings.rendementDefaults?.partMoyen ?? 20} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), partMoyen: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-emerald-400" />
                              <span className="text-[8px] text-slate-500">%</span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="bg-slate-950/40 border border-slate-800/50 rounded-2xl p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-[9px] font-black text-violet-400 uppercase tracking-wider">4 · Long Terme</span>
                          <button 
                            onClick={() => {
                              setGlobalSettings(prev => ({ ...prev, rendementDefaults: { ...(prev.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), rendementLong: 7.0, bonusLong: 0 } }));
                            }}
                            className="text-slate-600 hover:text-violet-400 text-[10px] px-1 transition-colors font-bold"
                            title="Réinitialiser hypothèses Long"
                          >↺</button>
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2.5">
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            <label className="text-[8px] font-bold text-slate-500 uppercase block">Rendement net</label>
                            <div className="flex items-center">
                              <input type="number" step="0.1" value={globalSettings.rendementDefaults?.rendementLong ?? 7.0} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), rendementLong: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-violet-400" />
                              <span className="text-[9px] text-slate-500 ml-0.5">%</span>
                            </div>
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            ...
                            <div className="flex items-center">
                              <span className="text-[9px] text-slate-500 mr-0.5">+</span>
                              <input type="number" step="0.1" value={globalSettings.rendementDefaults?.bonusLong ?? 0} onChange={(e) => setGlobalSettings(p => ({ ...p, rendementDefaults: { ...(p.rendementDefaults || FACTORY_SETTINGS.rendementDefaults), bonusLong: Number(e.target.value) } }))} className="w-full bg-transparent text-white font-black text-xs outline-none text-right focus:text-violet-400" />
                              <span className="text-[9px] text-slate-500 ml-0.5">%</span>
                            </div>
                          </div>
                          <div className="space-y-0.5 bg-slate-900/40 p-2 rounded-lg">
                            ...
                            <div className="flex items-center justify-end h-5">
                              <span className="text-xs font-black text-slate-500 select-none">
                                {Math.max(0, 100 - (globalSettings.rendementDefaults?.partCourt ?? 20) - (globalSettings.rendementDefaults?.partMoyen ?? 20))} %
                              </span>
                            </div>
                          </div>
                        </div>
                  ...
                  {/* SECTION PLEINE LARGEUR : BUDGET DÉTAILLÉ */}
                  <div className="lg:col-span-2 space-y-4 pt-4 border-t border-slate-800/80">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        <span className="w-6 h-px bg-slate-700" /> Budget Mensuel Détaillé par défaut
                      </h3>
                      <span className="text-[10px] font-bold text-slate-500">Sert de base à tous les nouveaux scénarios</span>
                    </div>

                    <div className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-6 backdrop-blur-sm grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                      {Object.keys(globalSettings.budget).map(key => (
                        <div key={key} className="bg-slate-950/40 border border-slate-800/50 rounded-xl p-2.5 flex flex-col justify-between space-y-1">
                          <label className="text-[8px] font-bold text-slate-400 uppercase tracking-tight truncate block" title={formatBudgetLabel(key)}>{formatBudgetLabel(key)}</label>
                          <div className="flex items-center gap-1">
                            <input 
                              type="number" 
                              ...
                              className="w-full bg-transparent text-white font-black text-xs outline-none focus:text-brand-primary"
                            />
                    ...
                    <div className="p-4 bg-brand-primary/5 border border-brand-primary/20 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center gap-4 justify-between">
                      <div className="flex items-center gap-3 text-brand-primary">
                        <AlertTriangle size={18} className="shrink-0" />
                        <p className="text-[10px] font-bold uppercase leading-relaxed max-w-2xl">
                          Ces valeurs globales initialisent l'application. En cliquant sur "Sauvegarder & Appliquer", le Scénario Actuel s'aligne automatiquement sur ces nouvelles références.
                        </p>
                      </div>
                      <button 
                        onClick={() => {
                        ...
                        className="px-6 py-2.5 bg-brand-primary text-white rounded-xl font-black text-xs uppercase tracking-wider hover:bg-brand-primary/90 transition-all shrink-0"
                      >
